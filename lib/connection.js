...
var memcached = new couchbase.Connection({host: 'localhost:8091', bucket: 'sockets'});
...
var memGet = memcached.get.bind(memcached);
memcached.get = function() {
  var options, callback;
  if (typeof arguments[1] === 'function') {
    options = null;
    callback = arguments[1];
  ...
  memGet(arguments[0], options, function(err, data) {
    if (err && err.code === NO_SUCH_KEY_CODE) return callback(null, null);
    callback(err, data);
  });
};
