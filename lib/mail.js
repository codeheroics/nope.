...
module.exports = {
  ...
  sendInvitationMail: function(email, referingUser, callback) {
    ...
    sendgrid.send({
      ...
      html:     '<p>' + referingUser.name + ' (' + referingUser.email + ') has invited you to join Nope.</p>' +
        '<p>Nope is a tug of war social game where you send "nopes" to your opponent until you get the last word!</p>' +
        '<p>To join, signup on https://www.nope.wtf !</p>' +
        '<p>Sincerely,</p><p>&nbsp;</p>' +
        '<p>The Chief Noper</p>',
      text: referingUser.name + ' (' + referingUser.email + ') has invited you to join Nope.\r\n\r\n' +
        'Nope is a tug of war social game where you send "nopes" to your opponent until you get the last word!\r\n' +
  ...
  sendForgottenPasswordEmail: function(email, token, callback) {
    ...
    sendgrid.send({
      to:       email,
      from:     'no-reply@nope.wtf',
      subject:  'Nope - Lost password',
      html:     '<p>You have requested a reset for your forgotten password at Nope.</p>' +
